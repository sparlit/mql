...
string Encrypt(string plaintext)
{
   ...
   // Encrypt
   if(CryptEncode(CRYPT_AES256, data, key, iv, result) <= 0) return "";
   ...
   // Base64 Encode
   uchar b64_result[];
   if(CryptEncode(CRYPT_BASE64, final_buf, key, iv, b64_result) <= 0) return "";
...
string Decrypt(string base64_ciphertext)
{
   ...
   if(CryptDecode(CRYPT_BASE64, b64_data, key, iv, raw_data) <= 0) return "";
   ...
   // Decrypt
   if(CryptDecode(CRYPT_AES256, ciphertext, key, iv, result) <= 0) return "";
